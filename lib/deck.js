...
function decodeEmbeddedDeckMarkup(html) {
  // < 等の16進エスケープは、ソース側のシリアライザ次第で大文字にも小文字にもなる
  return html
    .replace(/\\u003C/gi, "<")
    .replace(/\\u003E/gi, ">")
    .replace(/\\u002F/gi, "/")
...
export function extractDeckRows(deckText) {
  const rows = [];
  const normalized = String(deckText || "")
    .replace(/\\r\\n|\\n|\\r/g, "\n")
    .replace(/<\s*\/?\s*(main-deck|side-board)\s*>/gi, "\n")
    .replace(/<\s*(?:br|li|div)\b[^>]*>/gi, "\n")
    .replace(/<\s*\/\s*(?:li|div)\s*>/gi, "\n");
  ...
  for (const line of lines) {
    ...
    const name = match[2].replace(/\s+\([^)]+\)(?:\s+\S+)?$/g, "").trim();
    if (name && Number.isInteger(count) && count > 0 && count <= 250) rows.push({ name, count });
  }
  const merged = new Map();
  for (const row of rows) {
    if (!merged.has(row.name)) merged.set(row.name, { ...row });
    else merged.get(row.name).count += row.count;
  }
  return [...merged.values()];
}

export function extractDeckCardNames(deckText) {
  return extractDeckRows(deckText).map((row) => row.name);
}

function deckSections(body) {
  const mainMatch = String(body || "").match(/<main-deck\b[^>]*>([\s\S]*?)<\/main-deck>/i);
  const sideMatch = String(body || "").match(/<side-board\b[^>]*>([\s\S]*?)<\/side-board>/i);
  if (!mainMatch && !sideMatch) return { mainboard: extractDeckRows(body), sideboard: [] };
  return {
    mainboard: extractDeckRows(mainMatch?.[1] || ""),
    sideboard: extractDeckRows(sideMatch?.[1] || "")
  };
}

function deckListFields(mainboard, sideboard) {
  return {
    cards: [...new Set([...(mainboard || []), ...(sideboard || [])].map((row) => row.name))],
    mainboard,
    sideboard,
    mainboardCount: (mainboard || []).reduce((sum, row) => sum + row.count, 0),
    sideboardCount: (sideboard || []).reduce((sum, row) => sum + row.count, 0)
  };
}
...
function extractMtgTop8DeckRows(html) {
  const mainboard = [];
  const sideboard = [];
  const pattern = /<div\b(?=[^>]*\bclass\s*=\s*(?:"[^"]*\bdeck_line\b[^"]*"|'[^']*\bdeck_line\b[^']*'|[^\s>]*\bdeck_line\b))([^>]*)>([\s\S]*?)<\/div>/gi;
  let match;
  while ((match = pattern.exec(html))) {
    const attrs = match[1] || "";
    const zoneMatch = attrs.match(/\bid\s*=\s*(?:["']?)(md|sb)[^\s>"']*/i);
    if (!zoneMatch) continue;
    const nameMatch = match[2].match(/<span\b[^>]*class=["']?L14["']?[^>]*>([\s\S]*?)<\/span>/i);
    const countMatch = decodeHtmlText(match[2]).match(/^(\d+)\s+/);
    const name = decodeHtmlText(nameMatch?.[1] || "");
    const count = Number(countMatch?.[1]);
    if (!name || !Number.isInteger(count) || count <= 0 || count > 250) continue;
    (zoneMatch[1].toLowerCase() === "md" ? mainboard : sideboard).push({ name, count });
  }
  return deckListFields(mainboard, sideboard);
}
...
function mtgoDeckRows(rows) {
  return (Array.isArray(rows) ? rows : []).map((row) => ({
    name: String(row?.card_attributes?.card_name || "").trim(),
...
function extractMtgoDeckEntries(html, pageUrl, pageTitle, format = "") {
  ...
  return decks.map((deck, index) => {
    const list = deckListFields(mtgoDeckRows(deck.main_deck), mtgoDeckRows(deck.sideboard_deck));
    const { cards } = list;
    const player = String(deck.player || "").trim();
    const title = player ? `${eventName} - ${player}` : `${eventName} - Deck ${index + 1}`;
    // MTGO のタイトルは「イベント名 - プレイヤー名」でアーキタイプ情報を持たない。
    // タイトル推定は使わず Unknown に落とし、server.mjs のパス1〜3 (既知名照合・
    // プロファイル学習・土地色フォールバック) に分類を委ねる。
    return {
      title,
      archetype: inferArchetypeFromCards(cards, "Unknown"),
...
function deckListFormatMatches(attrs, requestedFormat) {
  ...
  if (!aliases.some((alias) => normalized === alias || normalized.includes(alias))) return false;
  // "Standard Brawl" 等は構築フォーマット名を含むが別フォーマット。要求側が brawl でない限り弾く
  if (/\bbrawl\b/.test(normalized) && !aliases.some((alias) => /brawl/.test(alias))) return false;
  return true;
}
...
export function extractDeckEntries(html, pageUrl, pageTitle, sourceUrls, pageDate = "", format = "") {
  ...
  while ((match = pattern.exec(decoded))) {
    ...
    const list = deckSections(body);
    const { cards } = deckListFields(list.mainboard, list.sideboard);
    ...
    entries.push({
      ...
      pageUrl,
      eventDate,
      ...deckListFields(list.mainboard, list.sideboard),
  ...
  if (!entries.length && isMtgTop8DeckPage(pageUrl)) {
    const list = extractMtgTop8DeckRows(html);
    const { cards } = list;
    entries.push({
      title: pageTitle,
  ...
  if (!entries.length && isDeckResultPage(pageUrl, sourceUrls)) {
    const mainboard = extractDeckRows(html);
    const list = deckListFields(mainboard, []);
    const { cards } = list;
    entries.push({
      title: pageTitle,
      archetype: inferArchetypeFromCards(cards, inferArchetype(pageTitle, pageTitle)),
