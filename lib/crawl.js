...
import { fetchTimeoutMs, seedPageMaxAgeMs } from "./config.js";
...
import { extractMtgTop8MetaSnapshot } from "./meta.js";
...
export async function crawlSources(sourceUrls, maxPages, options = {}) {
  ...
  const seedUrls = new Set(sourceUrls.filter(Boolean));

  async function processUrl(url) {
    try {
      console.log(`[crawl] fetching ${url}`);
      // シードページは新デッキの入口なので鮮度上限付き。子ページ（個別デッキ等）は不変なので無期限。
      const fetchOptions = seedUrls.has(url) ? { ...options, maxAgeMs: seedPageMaxAgeMs } : options;
      ...
      const metaSnapshot = extractMtgTop8MetaSnapshot(page.html, url, page.fetchedAt);
  ...
  while (queue.length > 0 && deckEntryCount < maxPages && pages.length < maxFetchPages) {
    ...
    for (const { page, newLinks, metaSnapshot } of results) {
      if (!page) continue;
      if (metaSnapshot) metaSnapshots.push(metaSnapshot);
  ...
  return { pages, errors, cacheStats, siteStats, deckEntryCount, knownArchetypes, metaSnapshots, unparsedDeckCount, sourceExhausted };
}
