...
export function displayCategory(item) {
  ...
  if (haystack.includes("impending")) return "待機カウンター";
  if (haystack.includes("omen")) return "龍兆";
...
export function tokenHints(card) {
  ...
  const patterns = [
    ...
    [/\bImpending\b/gi, "Impending（時間カウンター）"],
  ...
  for (const face of card.card_faces || []) {
    ...
    if (/omen/i.test(tl)) hints.push(`龍兆: ${face.name}`);
  }
...
export async function objectsForSource(source) {
  ...
  if (/\bImpending\b/i.test(text) && !produced.some((item) => /impending/i.test(item.name))) {
    produced.push(makeVirtualObject(source, "Marker", "Impending time counter", "インペンディング。時間カウンター付きで戦場に出る待機パーマネント。カウンター管理が必要。"));
  }
  ...
  for (const item of produced) {
    ...
    if (item.name === "Plotted card marker") item.note = "計画カード。除外ゾーンに表向きで置かれ後で無料キャスト。放送中は除外ゾーンの追跡が必要。";
    if (item.name === "Day/Night marker") item.note = "昼夜マーカー。現在が昼か夜かを追跡するマーカーが必要。";
    if (item.name === "Impending time counter") item.note = "インペンディング。時間カウンター付きで戦場に出る待機パーマネント。カウンター管理が必要。";
  }
