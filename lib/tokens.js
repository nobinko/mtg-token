...
export async function buildBulkObjects(matchedCards, options = {}) {
  const byKey = new Map();
  // 採用デッキ数の集計を、表示用 decks（36件に切り詰め）から分離するための台帳。
  // - urls: 発生源の decks に見えている URL の和集合。重複除去は正確だが、
  //   search 側で decks が24件に切り詰められるため、あふれた分の URL は見えない。
  // - overflow: その切り詰めであふれたデッキ数（source.deckCount - source.decks.length）。
  //   URL 不明で重複除去できず、発生源間で重複すると過大になりうる。
  // deckCount = urls.size + overflow。切り詰め長での頭打ちを避けることを優先した近似値。
  ...
  for (const source of matchedCards) {
    ...
    for (const object of objects) {
      ...
      for (const deck of sourceDecks) tally.urls.add(deck.url);
      tally.overflow += Math.max(0, (source.deckCount || 0) - sourceDecks.length);
