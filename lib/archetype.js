...
// MTGO イベント語彙 (Challenge 64, League, Super Qualifier 等) もノイズ扱いし、
// 大会名がアーキタイプ名として残らないようにする。
const NOISE_WORDS = /\b(top\s*\d*|rank\s*\d*|decklist|decklists|standard|pioneer|modern|legacy|event|championship|regional|spotlight|platinum|mythic|rank\s+player|magic\s+play|coverage|player|challenge\s*\d*|league\s*\d*|showcase|super\s+qualifier|qualifiers?|preliminary|prelims?|last\s+chance|mtgo)\b/gi;
// 大会タイトルに含まれる日付 (2026-07-13, 07/13/26 等)。アーキタイプ名には現れない。
const DATE_TOKENS = /\b(?:\d{4}[-\/.]\d{1,2}[-\/.]\d{1,2}|\d{1,2}\/\d{1,2}\/\d{2,4})\b/g;
...
export function inferArchetype(title, fallback = "") {
  ...
  // 色語・戦略語のない先頭断片は、Belcher のような固有名 (文字2字以上) だけ通し、
  // ノイズ除去後に残った日付・番号の残骸は Unknown に落として色フォールバックに委ねる。
  const best = parts.find((p) => COLOR_WORDS.test(p) && p.length < 60)
    ?? parts.find((p) => STRATEGY_WORDS.test(p) && p.length < 60)
    ?? ((parts[0].match(/\p{L}/gu) || []).length >= 2 ? parts[0] : null);
