import { environmentUpdates, confirmedEvents } from "./updates.js";
...
export function mergeEnvironmentEvents(manualEvents, autoEvents) {
  ...
  for (const event of autoEvents || []) {
    const overriddenSet = event.setCode && manual.some((item) => item.type === "set-release" && item.setCode === event.setCode);
    if (!overriddenSet && !manualKeys.has(`${event.date}|${event.type}`)) merged.push(event);
  }
  return merged;
}
...
export async function formatEnvironmentInfo(format, targetDate) {
  ...
  const pending = active.pack.events.some((event) => event.autoDetected && event.confirmed !== true && event.formatsAffected.includes(format) && event.date <= targetDate);
  if (pending) return { format, targetDate, resolved: false, startDate: "", reason: "このフォーマットに適用日未確認の禁止改定があります。配信元での確認後、環境データを更新してください。" };
  const fresh = active.checkedAt && Date.now() - Date.parse(active.checkedAt) < 24 * 60 * 60 * 1000;
  const events = mergeEnvironmentEvents(confirmedEvents(active.pack), fresh ? active.setEvents : await fetchSetReleaseEvents());
  return { ...resolveEnvironmentFromEvents(events, format, targetDate), events, dataRevision: active.revision, checkedAt: active.checkedAt,
    temporalNote: "候補カードは取得時点の使用可否に基づきます。過去・将来の大会日時点の使用可否を再現するものではありません。" };
}
