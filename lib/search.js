import { escapeRegex, imageRefFor, officialExpansionCode, officialExpansionName } from "./util.js";
...
export function findCardMentions(cards, pages) {
  ...
  for (const card of cards) {
    ...
    for (const deck of deckEntries) {
      if (findNameInDeck(names, deck)) {
        mentionedDecks.push({
          ...
          archetypeIdentity: deck.archetypeIdentity || null,
          url: deck.url,
    ...
    if (mentionedSources.length) {
      ...
      const imageRef = imageRefFor(card);
      results.push({
        id: card.id,
        raw: card,
        name: card.name,
        ...
        deckUrls: uniqueDecks.map((deck) => deck.url),
