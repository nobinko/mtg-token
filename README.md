...
- MTGTop8の直近メタ上位10件を、母数・集計期間・取得日時付きで表示
- 上位アーキタイプごとに直近の完全リストを比較し、典型例となる実在の60+15枚リストを表示・Arena形式でコピー
- 典型リストを画像付きのプレイテスト卓へ展開。自分／相手のライフ・毒、土地／クリーチャー／その他の戦場レーン、ドラッグ移動と並べ替え、90度タップ、フェッチ、サーチと上からX枚、カウンター、トークン、サイド、保存・再開、Undo/Redo、操作ログを使ってひとり回し
...
- トップメタ順位は検索対象デッキの自動分類ではなく、MTGTop8が現在公開している直近集計です。過去の大会日を指定しても、その当時のメタ順位にはなりません。
- ひとり回しは紙の操作を再現する手動プレイテスト卓です。カード効果、マナ、優先権、勝敗、対戦相手は自動処理しません。盤面と最大60件の操作履歴はデッキごとにブラウザへ保存されます。
